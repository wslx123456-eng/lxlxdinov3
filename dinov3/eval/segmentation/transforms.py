...
class FixedSideResize:
    ...
    def _resize(self, img, label, small_size):
        init_height, init_width = _get_hw(img)
    ...
    def forward(self, img, label):
        ...
        if self.cat_max_ratio < 1.0:
            ...
            for _ in range(10):
                ...
                labels, cnt = np.unique(seg_temp.cpu().numpy(), return_counts=True)
...
class NormalizeImage(torch.nn.Module):
    ...
    def forward(self, img, label):
        ...
        if len(self.mean) == 1:
            mean = torch.full((n_channels,), float(self.mean[0]), dtype=img.dtype, device=img.device)
        else:
            mean = torch.tensor(self.mean, dtype=img.dtype, device=img.device)
        if len(self.std) == 1:
            std = torch.full((n_channels,), float(self.std[0]), dtype=img.dtype, device=img.device)
        else:
            std = torch.tensor(self.std, dtype=img.dtype, device=img.device)
        if mean.numel() != n_channels or std.numel() != n_channels:
            raise ValueError(f"Mean/std channels ({mean.numel()}, {std.numel()}) do not match image channels ({n_channels})")
        mean = mean[:, None, None]
        std = std[:, None, None]
        return (img - mean) / std, label


class ToImageTensor(torch.nn.Module):
    def forward(self, img, label):
        return _to_image_tensor(img), label
    ...
    def forward(self, img, label):
        ...
        if isinstance(label, np.ndarray):
            ...
            if label.ndim == 3 and label.shape[0] == 1:
                label = label[0]
            return img, Mask(label)
        return img, Mask(np.array(label, copy=True))


class RemapMaskValues(torch.nn.Module):
    def __init__(self, label_values: Sequence[int], ignore_index: int = 255):
        super().__init__()
    ...
    def forward(self, img, label):
        ...
        remapped = torch.full_like(label_tensor, fill_value=self.ignore_index)
        for class_index, raw_value in enumerate(self.label_values):
            remapped[label_tensor == raw_value] = class_index
        return img, Mask(remapped)
...
def make_segmentation_train_transforms(
    *,
    ...
    enable_photometric_distortion: bool = True,
    ...
    if label_values:
        transforms_list.append(RemapMaskValues(label_values=label_values))
    # Resizing
    ...
    transforms_list.extend([ToImageTensor()])
    ...
    transforms_list.extend(
        [
            ...
            *([PhotoMetricDistortion()] if enable_photometric_distortion else []),
...
def make_segmentation_eval_transforms(
    *,
    ...
    transforms_list.append(TransformImages(transforms=[ToImageTensor(), NormalizeImage(mean=mean, std=std)]))
