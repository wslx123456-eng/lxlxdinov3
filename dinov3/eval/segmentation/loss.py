...
class CrossEntropyLoss(nn.Module):
    ...
    def forward(self, pred, label):
        ...
        if class_weight is not None and not isinstance(class_weight, torch.Tensor):
            class_weight = pred.new_tensor(class_weight, dtype=pred.dtype)
        elif isinstance(class_weight, torch.Tensor):
            class_weight = class_weight.to(device=pred.device, dtype=pred.dtype)
...
class MultiSegmentationLoss(nn.Module):
    ...
    def __init__(self, diceloss_weight=0.0, celoss_weight=0.0, class_weights=None):
        ...
        if celoss_weight > 0:
            self.losses.append(
                CrossEntropyLoss(
                    ...
                    class_weight=get_class_weight(class_weights),
                )
            )
    ...
    def forward(self, pred, gt):
        ...
        return sum(loss_fn(pred, gt) for loss_fn in self.losses)
